...
export default function LandingPage() {
  ...
  const rawCategories = useQuery(api.categories.listPublic);
  ...
  const promoCards = useMemo(() => {
    ...
    return selected.map((product, index) => ({
      ...
      href: index === 0 ? "#collections" : `/urun?id=${product._id}`,
      product,
    }));
  }, [products]);
    ...
    <main className="bg-paper pb-12 text-navy sm:pb-16">
      ...
      <section id="collections" className="py-8 sm:py-10">
        <div className="container mx-auto px-4">
          ...
          {cartStatus ? <p className="mt-4 text-sm text-denim">{cartStatus}</p> : null}

          {catalog.length > 0 ? (
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {catalog.map((product) => (
                <article key={product._id} className="flex flex-col border border-navy/10 bg-paper">
                  <Link href={`/urun?id=${product._id}`} className="group relative block h-64 overflow-hidden sm:h-72">
                    <Image
                      src={product.image}
                      alt={product.name}
                      fill
                  ...
                  <div className="flex flex-1 flex-col gap-3 p-4">
                    ...
                    <div className="mt-auto flex gap-2">
                      <Button asChild size="sm" variant="outlineGold" className="flex-1">
                        <Link href={`/urun?id=${product._id}`}>Incele</Link>
                      </Button>
                      <Button size="sm" className="flex-1" onClick={() => void onAddToCart(product._id)}>
                        Sepete Ekle
                      </Button>
                    </div>
                  </div>
          ...
          {products.length > 0 ? (
            <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {products.slice(0, 3).map((product) => (
                <Link key={`pick-${product._id}`} href={`/urun?id=${product._id}`} className="group overflow-hidden border border-navy/10 bg-paper">
